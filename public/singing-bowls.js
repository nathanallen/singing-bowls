...
var graphView = (function(){
  ...
  return function(freq, amp){
    ...
    old = els[freq].getAttribute("data-amp") || amp;

    avg = Math.floor( (+old + amp)/2 );
    els[freq].setAttribute("style", `height:${avg}px;`);
    els[freq].setAttribute("data-freq", freq);
    els[freq].setAttribute("data-amp", avg);
  }
}())

function FrequencySampler(osc, audioIn, stepSize=1, offset=100, max=1200) {
